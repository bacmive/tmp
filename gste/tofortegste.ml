...
let tag_inv_to_bit_form form args concret_list =
		...
		| Eqn (e1, e2) -> (
			...
			| (IteForm (f, e11, e12), Const (SymbIntC (c_name, data_size))) -> (
				match f,e11 with
				| (Eqn (fe1, fe2), IVar (Para (v, Const (IntC (i, index_size))))) -> (
					match fe1, fe2 with
					|(Const (IntC(value, index_size1)), Const (IntC (i2, index_size2))) -> (
						termForm2bitForm (Eqn (IVar (Para (v, Const (IntC (value, index_size1)))),  Const (SymbIntC (c_name, data_size))))
					)
					| (IVar (Ident (var_name, Int index_size1)), Const (IntC (i2, index_size2))) when (expr_in_args fe1 args)-> (
						termForm2bitForm (Eqn (IVar (Para (v, Const (IntC ((Hashtbl.find hash fe1), index_size1)))), Const (SymbIntC (c_name, data_size))))
					)
					| (Uif (op_str, opex1::opex2::[]), Const (IntC (i2, index_size2))) when ((expr_in_args opex1 args)|| (expr_in_args opex2 args)) ->(
						...
						| (IVar (Ident (opex1_name, Int opex1_size)), IVar (Ident (opex2_name, Int opex2_size))) when ((expr_in_args opex1 args) && (expr_in_args opex2 args))->(
							termForm2bitForm (Eqn (IVar (Para (v, Const (IntC (((Hashtbl.find hash opex1)+(Hashtbl.find hash opex2)), opex1_size)))), Const (SymbIntC (c_name, data_size))))
						)
						| (IVar (Ident (opex1_name, Int opex1_size)), Const (IntC (opex2_value, opex2_size))) -> (
							termForm2bitForm (Eqn (IVar (Para (v, Const (IntC (((Hashtbl.find hash opex1)+ opex2_value), opex1_size)))), Const (SymbIntC (c_name, data_size))))
						)
						| (Const (IntC (opex1_value, opex1_size)),IVar (Ident (opex2_name, Int opex2_size))) -> (
							termForm2bitForm (Eqn (IVar (Para (v, Const (IntC (((Hashtbl.find hash opex2)+ opex1_value), opex1_size)))), Const (SymbIntC (c_name, data_size))))
						)
...
let toSTEfl model_name gs tag binNodes=   
	let main_assertion_graph init_node node_set edge_set =
		match init_node with (Vertex inum) -> Printf.fprintf stdout "let vertexI = Vertex %d;\n" inum ;
		Printf.fprintf stdout "%s" ("let vertexL = [" ^ (String.concat "," (List.map (fun (Vertex i) -> Printf.sprintf "Vertex %d" i) node_set)) ^ "];\n" );
		Printf.fprintf stdout "%s" ("let edgeL = [" ^ (String.concat "," (List.map (fun (Edge ((Vertex f),(Vertex t))) -> Printf.sprintf "Edge (Vertex %d) (Vertex %d)" f t) edge_set)) ^ "];\n");
